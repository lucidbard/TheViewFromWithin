...
public class sbRemote
{
	...
	public const float DEAD_ZONE = 1000f;
	...
	public const float JOYSTICK_VERTICAL_BIAS = 6000f;
	public const float JOYSTICK_HORIZONTAL_BIAS = -600f;
	...
	public static void remoteLateUpdate ()
	{
		...
		if(remoteStatus&& remoteData!=IntPtr.Zero && dataAvail) {
			...
			sbCursors.cursorScreenPosition.x = System.BitConverter.ToInt16(myBytes,24);
			sbCursors.cursorScreenPosition.y = System.BitConverter.ToInt16(myBytes,26);
			
			sbCursors.trackerRadius = System.BitConverter.ToInt16(myBytes,28);
			//SetDepth(trackerRadius);
		...
		#endif
		//		if (seebrightCursor3D != null && metTracker != null)
		//			seebrightCursor3D.transform.localPosition = metTracker.transform.position;
	...
	public static float GetAxis (string axis)
	{
		...
		switch (axis) {
			#if (UNITY_IPHONE || UNITY_ANDROID) && !UNITY_EDITOR    
		case JOY_HORIZONTAL:
			if(Mathf.Abs(joy_x)>DEAD_ZONE)
				retVal = (joy_x+JOYSTICK_HORIZONTAL_BIAS)/MAX_JOY_HORIZONTAL;
			else
				retVal = 0;
			Mathf.Clamp(retVal,-30000,30000);
			break;
		case JOY_VERTICAL:
			if(Mathf.Abs(joy_y)>DEAD_ZONE)
				retVal = (joy_y+JOYSTICK_VERTICAL_BIAS)/MAX_JOY_VERTICAL;
			else
				retVal = 0;
	...
	{
		return 
			"JH:" + GetAxis (JOY_HORIZONTAL) * (float)MAX_JOY_HORIZONTAL +
				"JV:" + GetAxis (JOY_VERTICAL) * (float)MAX_JOY_VERTICAL +
